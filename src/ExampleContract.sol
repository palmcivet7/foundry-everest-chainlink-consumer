...
contract ExampleContract is IExampleContract, Ownable {
    ...
    constructor(address _everestConsumer) {
        ...
        LinkTokenInterface(everestConsumer.linkAddress()).approve(_everestConsumer, type(uint256).max);
    }
    ...
    function requestVerification(address _whose) external override onlyOwner {
        IERC20(everestConsumer.linkAddress()).safeTransferFrom(
            msg.sender, address(this), everestConsumer.oraclePayment()
        );
    ...
    function getLatestVerification(address _whose) external view override returns (KYCResponse memory kycResponse) {
        ...
        IEverestConsumer.Request memory request = everestConsumer.getRequest(requestId);
